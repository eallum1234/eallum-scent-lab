...
import { stepCopyFor } from "../data/translations";

export default function ProgressRail({ language, stepIndex }) {
  return (
    <aside className="rounded-md border border-[#ddd4c4] bg-white p-4 shadow-soft">
      {STEPS.map((step, index) => {
        const translated = stepCopyFor(step.key, language);
        return (
          <div
            key={step.key}
